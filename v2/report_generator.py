    ...
    if include_action_plan:
        ...
        if calendar:
            for week in calendar:
                wk = week.get("week") or week.get("label") or "Week"
                parts.append(_heading(str(wk), 3))
                launches = week.get("launch") or []
                if launches:
                    for item in launches:
                        if isinstance(item, dict):
                            label = item.get("test_name") or item.get("page") or "—"
                        else:
                            label = str(item)
                        parts.append(_bullet(f"Launch: {label}"))
                else:
                    parts.append(_para("No launches scheduled.", italic=True))
                parts.append(_spacer())
        else:
